...
const plantsApp = angular.module('plantsApp', ['ui.bootstrap', 'chart.js'])
...
plantsApp.controller('PlantsCtrl', function ($scope, $http, $uibModal) {
  ...
  $scope.daysSinceWater = function (plant) {
    ...
    const plantDate = new Date(plant.dates[plant.dates.length - 1])
    const today = new Date()
    today.setHours(0, 0, 0, 0)

    const days = Math.round((today - plantDate) / (1000 * 60 * 60 * 24))
    return {
      text: days,
    ...
    modalInstance.result.then(function (status) {
      ...
      if (status === 'deleted') {
        for (let i = 0; i < $scope.places.length; i++) {
          const deletedIndex = $scope.places[i].plants.findIndex(function (e) {
  ...
  $scope.sort = function () {
    for (let i = 0; i < $scope.places.length; i++) {
      $scope.places[i].plants.sort(function (a, b) {
        const ad = $scope.daysSinceWater(a).days
        const bd = $scope.daysSinceWater(b).days
        return bd - ad
      })
    }
  }
...
plantsApp.controller('PlantStatisticsInstanceCtrl', function ($modalInstance, plant) {
  ...
  const dayDiffs = []
  let prev = null
  for (let i = 0; i < plant.dates.length; i++) {
    if (prev !== null) {
      const dayDiff = (new Date(plant.dates[i]) - prev) / (24 * 3600 * 1000)
  ...
  // Median.
  const median = (function (diffs) {
    if (diffs.length == 0) return 0.0
    const sorted = diffs.slice(0) // clone
  ...
  const shortest = Math.min.apply(self, dayDiffs)
  const longest = Math.max.apply(self, dayDiffs)

  // Standard deviation of time between water.
  const stddev = (function (diffs, avg) {
    let sum = 0
    for (let i = 0; i < diffs.length; i++) {
  ...
  this.histogram = (function (diffs) {
    ...
    const data = keys.map(function (e) {
      return counts[e]
    })
